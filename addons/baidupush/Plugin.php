...
class Plugin extends Addons    // 需继承think\Addons类
{
    ...
    public function run()
    {
        ...
        if (empty($push_num)) {
            $list = Db::name('addons_baidupush')->field('id,url')->where(['dateymd' => date('Ymd'), 'status' => 0, ['num', '<=', 5]])->limit(200)->select()->toArray();
            if (!empty($list)) {
                ...
                try {
                    ...
                    if ($result) {
                        ...
                        if ($res && isset($res['success']) && $res['success']) {
                            ...
                            Db::name('addons_baidupush')->where('id', 'in', $in)->inc('num')->strict(false)->field(true)->update(['remain' => 0, 'result' => isset($res['message']) ?: json_encode($res), 'update_time' => time(), 'status' => 0]);
                        }
